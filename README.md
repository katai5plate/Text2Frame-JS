...
- `cmd.etc.Script` は `function () {}` または `() => {}` の直接入力を行い、外部の変数などを含まないようにしてください。
  - 内部的には、引数指定された関数を `toString()` しているためです。`Game_Interpreter` 内でアクセスできない変数は使えません。
- 条件分岐イベントコマンドは `cmd.flow.Check` のみです。スクリプト文字列で条件を指定してください。
...
- コンソールから直接コマンドを試し書きできる機能。または REPL 的なツール。
  - 結局打ち込んでみないとわからないことが多いので、書いて動かすまでのフローを限りなく短くしたい。
  - REPL 的なツールを作るなら、左半分に [VSCode 風エディタ](https://microsoft.github.io/monaco-editor/)、右上に変換されたテキスト、右下にツクールの実行環境って感じにしたい。
    - 実行環境はイベントコマンドエディタの「テスト」を実行した時みたいな感じ
